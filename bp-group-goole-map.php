...
private function BPSimpleGoogleMap(){
    $this->__construct();
}
function get_instance(){
  if(!isset(self::$instance)){
      self::$instance=new BPSimpleGoogleMap();
  }
 return self::$instance;   
}   

//laod group extension
function load_core(){
...
function widget($args, $instance){
    if(!(bp_is_group()&&!BPSimpleGoogleMapGroupHelper::is_map_disabled()))
       return;//currently we are only supporting the group maps
    //display map
...
<p>
       ...
       <p>
            <label for="bp-simple-google-map-type"><?php _e('Map Type', 'bpsgmap'); ?> 
                <select class="widefat" id="<?php echo $this->get_field_id( 'map_type' ); ?>" name="<?php echo $this->get_field_name( 'map_type' ); ?>" style="width: 30%" >
                    <option value='roadmap' <?php if($map_type=='roadmap') echo 'selected="selected"';?>><?php _e('Roadmap','bpsgmap');?></option>
                    <option value='satellite' <?php if($map_type=='satellite') echo 'selected="selected"';?>><?php _e('Satellite','bpsgmap');?></option>
                    <option value='terrain' <?php if($map_type=='terrain') echo 'selected="selected"';?>><?php _e('Terrain','bpsgmap');?></option>
                    <option value='hybrid' <?php if($map_type=='hybrid') echo 'selected="selected"';?>><?php _e('Hybrid','bpsgmap');?></option>
                </select>             
            </label>
        </p>
	<p>
            <label for="bp-simple-google-map-zoom-level"><?php _e('Zoom Level', 'bpsgmap'); ?> 
                <select class="widefat" id="<?php echo $this->get_field_id( 'map_zoom_level' ); ?>" name="<?php echo $this->get_field_name( 'map_zoom_level' ); ?>" style="width: 30%" >
                    <?php for ($i=0;$i<=21;$i++):?>
	...
	<p>
            <label for="bp-simple-google-map-width"><?php _e('Map Width', 'bpsgmap'); ?>
                <input class="widefat" id="<?php echo $this->get_field_id( 'map_width' ); ?>" name="<?php echo $this->get_field_name( 'map_width' ); ?>" type="text" size="3" value="<?php echo $map_width;?>" /> px
            </label>
        </p>
        <p>
            <label for="bp-simple-google-map-height"><?php _e('Map Height(max 640)', 'bpsgmap'); ?>
                <input class="widefat" id="<?php echo $this->get_field_id( 'map_height' ); ?>" name="<?php echo $this->get_field_name( 'map_height' ); ?>" type="input" size="3" value="<?php echo $map_height;?>"  />px
            </label>
        </p>
        <?php
}
...
function group_disable_form(){?>
	...
	<label><input type="checkbox" name="group-disable-bpsgmap" id="group-disable-bpsgmap" value="yes" <?php if(self::is_map_disabled()):?> checked="checked"<?php endif;?>/> <?php _e( 'Disable Map', 'bpsgmap' ) ?></label>
    </div>
<?php
